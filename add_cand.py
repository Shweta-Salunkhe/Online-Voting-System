...
frame_alpr = tk.LabelFrame(window, text=" --Process-- ", width=600, height=450, bd=5, font=('times', 15, ' bold '),bg="pink")
...
def show():
    global file
    file = askopenfilename(initialdir=r'D:/Smart_Voting_System', title='Select Image',
                                       filetypes=[("all files", "*.*")])
...
def insertBLOB():
    ...
    ms.showinfo("Driver Licence Record","Registration Completed !!")
    window.destroy()
